export const SQL =[
    ...
    },
    {
      ...
      arguments: [
        { name: "% (percent)", description: "Заменяет любое количество символов (0 и более).", example: "WHERE email LIKE '%@gmail.com'" },
        { name: "_ (underscore)", description: "Заменяет ровно один любой символ.", example: "WHERE code LIKE 'A_123' -- например AB123 или AC123" },
        { name: "ILIKE", description: "Регистронезависимый поиск (в PostgreSQL).", example: "WHERE name ILIKE 'ivan%'" }
      ]
    },
    {
      id: "exists",
      ...
      arguments: [
        { name: "correlation", description: "Обычно используется с коррелированными подзапросами.", example: "WHERE EXISTS (SELECT 1 FROM orders o WHERE o.user_id = u.id)" },
        { name: "efficiency", description: "Останавливает поиск, как только найдена хотя бы одна строка.", example: "SELECT 'Found' WHERE EXISTS (SELECT * FROM big_table LIMIT 1)" },
        { name: "not exists", description: "Проверка на отсутствие записей.", example: "WHERE NOT EXISTS (SELECT 1 FROM blacklist b WHERE b.email = u.email)" }
      ]
    },
    {
      id: "all_operator",
      name: "ALL",
      category: "Логические операторы",
      description: "Сравнивает значение со всеми значениями в подзапросе. Условие истинно, если оно верно для всех строк.",
      syntax: "column comparison_operator ALL (subquery);",
      example: "SELECT name FROM products WHERE price > ALL (SELECT price FROM competitors);",
      arguments: [
        { name: "comparison", description: "Используется с =, <>, <, <=, >, >=.", example: "WHERE salary >= ALL (subquery)" },
        { name: "subquery", description: "Подзапрос должен возвращать один столбец.", example: "WHERE id <> ALL (SELECT id FROM deleted_users)" },
        { name: "empty set", description: "Если подзапрос пуст, ALL всегда возвращает TRUE.", example: "WHERE score > ALL (SELECT score FROM empty_table)" }
      ]
    },
    {
      ...
      name: "ANY | SOME",
      category: "Логические операторы",
      description: "Сравнивает значение с любым из значений в подзапросе. Истинно, если хотя бы для одного значения условие верно.",
      syntax: "column comparison_operator ANY (subquery);",
      example: "SELECT name FROM employees WHERE salary > ANY (SELECT salary FROM interns);",
      arguments: [
        { name: "equivalence", description: "SOME — это синоним ANY.", example: "WHERE price = ANY (SELECT p FROM prices)" },
        { name: "in-like", description: "= ANY эквивалентно оператору IN.", example: "WHERE id = ANY (1, 2, 3)" },
        { name: "comparison", description: "Позволяет использовать операторы больше/меньше для списка.", example: "WHERE quantity < ANY (SELECT stock FROM inv)" }
      ]
    },
    // Арифметические операторы
    {
      ...
      arguments: [
        { name: "columns", description: "Сложение значений двух столбцов.", example: "SELECT salary + bonus FROM employees;" },
        { name: "literals", description: "Прибавление константы.", example: "SELECT score + 10 FROM results;" },
        { name: "intervals", description: "В некоторых СУБД — прибавление времени к дате.", example: "SELECT created_at + INTERVAL '1 day' FROM events;" }
      ]
    },
    {
      id: "subtract",
      ...
      arguments: [
        { name: "difference", description: "Нахождение разницы между полями.", example: "SELECT end_time - start_time FROM logs;" },
        { name: "negative", description: "Использование как унарного минуса.", example: "SELECT -balance FROM accounts;" },
        { name: "dates", description: "Вычитание дат для получения количества дней.", example: "SELECT '2024-01-01'::date - '2023-12-01'::date;" }
      ]
    },
    {
      id: "multiply",
      ...
      arguments: [
        { name: "percentage", description: "Расчет процентов.", example: "SELECT total * 0.15 AS vat FROM invoices;" },
        { name: "scaling", description: "Изменение масштаба значений.", example: "SELECT weight_kg * 1000 AS weight_g FROM items;" },
        { name: "multiple", description: "Цепочка умножений.", example: "SELECT length * width * height FROM boxes;" }
      ]
    },
    {
      id: "divide",
      ...
      arguments: [
        { name: "integer", description: "При делении целых чисел результат часто тоже целый.", example: "SELECT 5 / 2; -- может вернуть 2" },
        { name: "float", description: "Для точного деления один операнд должен быть дробным.", example: "SELECT 5.0 / 2; -- вернет 2.5" },
        { name: "zero", description: "Деление на ноль вызывает ошибку в большинстве СУБД.", example: "SELECT amount / NULLIF(qty, 0);" }
      ]
    },
    {
      id: "modulo",
      ...
      arguments: [
        { name: "parity", description: "Проверка на четность/нечетность.", example: "WHERE id % 2 = 0 -- только четные ID" },
        { name: "cycling", description: "Используется для циклического перебора значений.", example: "SELECT (counter % 10) + 1;" },
        { name: "distribute", description: "Распределение строк по группам (шардинг).", example: "WHERE id % 4 = 1" }
      ]
    },
    // Операторы сравнения
    {
      ...
      arguments: [
        { name: "strings", description: "Сравнение текстовых строк (чувствительность к регистру зависит от БД).", example: "WHERE status = 'active'" },
        { name: "numbers", description: "Сравнение числовых данных.", example: "WHERE price = 99.99" },
        { name: "nulls", description: "Внимание: для NULL используйте IS NULL, а не = NULL.", example: "WHERE parent_id IS NULL" }
      ]
    },
    {
      id: "not_equal",
      ...
      arguments: [
        { name: "<>", description: "Стандартный SQL оператор неравенства.", example: "WHERE type <> 'internal'" },
        { name: "!=", description: "Более современный синтаксис, поддерживаемый большинством СУБД.", example: "WHERE status != 'deleted'" },
        { name: "filtering", description: "Исключение конкретных значений из результата.", example: "WHERE id <> 1 -- вернуть всех кроме первого" }
      ]
    },
    {
      id: "greater_than",
      ...
      arguments: [
        { name: "numbers", description: "Сравнение числовых величин.", example: "WHERE stock > 0" },
        { name: "dates", description: "Выбор записей позже определенной даты.", example: "WHERE created_at > '2024-01-01'" },
        { name: "sorting", description: "Часто используется в условиях фильтрации перед сортировкой.", example: "WHERE rank > 10 ORDER BY rank" }
      ]
    },
    {
      id: "less_than",
      ...
      arguments: [
        { name: "thresholds", description: "Проверка пороговых значений.", example: "WHERE temperature < 0" },
        { name: "strings", description: "Лексикографическое сравнение (по алфавиту).", example: "WHERE name < 'B' -- имена на букву A" },
        { name: "intervals", description: "Сравнение с расчетными интервалами.", example: "WHERE duration < INTERVAL '1 hour'" }
      ]
    },
    {
      ...
      arguments: [
        { name: "ranges", description: "Часто используется для задания нижней границы диапазона.", example: "WHERE age >= 18" },
        { name: "pagination", description: "Выборка начиная с определенного ID.", example: "WHERE id >= 101 LIMIT 10" },
        { name: "inclusive", description: "Включает само сравниваемое значение в результат.", example: "WHERE price >= 10 -- 10 тоже попадет" }
      ]
    },
    {
      ...
      arguments: [
        { name: "deadlines", description: "Выбор записей до определенного момента включительно.", example: "WHERE due_date <= CURRENT_DATE" },
        { name: "limitations", description: "Ограничение верхней границы.", example: "WHERE attempts <= 3" },
        { name: "combinations", description: "Используется вместе с >= для замены BETWEEN.", example: "WHERE price >= 10 AND price <= 20" }
      ]
    },
    // Операторы множеств
    {
      ...
      description: "Объединяет результаты двух SELECT запросов в один набор, удаляя дубликаты.",
      syntax: "SELECT ... UNION SELECT ...",
      example: "SELECT name FROM customers UNION SELECT name FROM suppliers;",
      arguments: [
        { name: "columns", description: "Количество и типы столбцов в обоих запросах должны совпадать.", example: "SELECT id, name FROM t1 UNION SELECT id, name FROM t2" },
        { name: "distinct", description: "UNION по умолчанию выполняет DISTINCT.", example: "SELECT city FROM users UNION SELECT city FROM offices" },
        { name: "order_by", description: "Сортировка применяется ко всему итоговому набору в конце.", example: "SELECT n FROM a UNION SELECT n FROM b ORDER BY n" }
      ]
    },
    {
      id: "union_all",
      name: "UNION ALL",
      category: "Операторы множеств",
      description: "Объединяет результаты запросов, включая все дубликаты (быстрее, чем UNION).",
      syntax: "SELECT ... UNION ALL SELECT ...",
      example: "SELECT city FROM stores UNION ALL SELECT city FROM warehouses;",
      arguments: [
        { name: "performance", description: "Работает быстрее UNION, так как не ищет дубликаты.", example: "SELECT log_msg FROM logs_old UNION ALL SELECT log_msg FROM logs_new" },
        { name: "duplicates", description: "Сохраняет все строки, даже если они идентичны.", example: "SELECT user_id FROM clicks UNION ALL SELECT user_id FROM views" },
        { name: "reporting", description: "Используется, когда важно сохранить полную статистику.", example: "SELECT total FROM sales_q1 UNION ALL SELECT total FROM sales_q2" }
      ]
    },
    {
      id: "intersect",
      name: "INTERSECT",
      category: "Операторы множеств",
      description: "Возвращает только те строки, которые присутствуют в обоих результатах запросов.",
      syntax: "SELECT ... INTERSECT SELECT ...",
      example: "SELECT user_id FROM active_subscribers INTERSECT SELECT user_id FROM newsletter_readers;",
      arguments: [
        { name: "overlap", description: "Находит пересечение двух множеств.", example: "SELECT product_id FROM promo_a INTERSECT SELECT product_id FROM promo_b" },
        { name: "requirements", description: "Как и в UNION, структуры таблиц должны быть идентичны.", example: "SELECT email FROM staff INTERSECT SELECT email FROM customers" },
        { name: "distinct", description: "Обычно возвращает только уникальные строки.", example: "SELECT tag FROM post1_tags INTERSECT SELECT tag FROM post2_tags" }
      ]
    },
    {
      id: "minus_except",
      name: "MINUS/EXCEPT",
      category: "Операторы множеств",
      description: "Возвращает строки из первого запроса, которых нет во втором (EXCEPT в PostgreSQL/SQL Server, MINUS в Oracle).",
      syntax: "SELECT ... EXCEPT SELECT ... -- или MINUS",
      example: "SELECT id FROM all_products EXCEPT SELECT product_id FROM sales;",
      arguments: [
        { name: "EXCEPT", description: "Синтаксис для PostgreSQL и SQLite.", example: "SELECT id FROM t1 EXCEPT SELECT id FROM t2" },
        { name: "MINUS", description: "Синтаксис для Oracle (и некоторых других).", example: "SELECT id FROM t1 MINUS SELECT id FROM t2" },
        { name: "exclusion", description: "Позволяет легко найти 'недостающие' или 'уникальные для первой таблицы' записи.", example: "SELECT email FROM users EXCEPT SELECT email FROM unsubscribed" }
      ]
    },
    // Операторы JOIN
    {
      ...
      description: "Возвращает записи, имеющие совпадающие значения в обеих таблицах.",
      ...
      arguments: [
        { name: "ON", description: "Условие связывания таблиц.", example: "JOIN profiles p ON u.id = p.user_id" },
        { name: "multiple", description: "Можно последовательно объединять несколько таблиц.", example: "SELECT * FROM t1 JOIN t2 ON ... JOIN t3 ON ..." },
        { name: "filtering", description: "Строки без совпадений отсекаются из результата.", example: "JOIN categories c ON p.cat_id = c.id -- только товары с категориями" }
      ]
    },
    {
      id: "left_join",
      name: "LEFT JOIN",
      category: "Операторы JOIN",
      description: "Возвращает все записи из левой таблицы и совпадающие записи из правой (или NULL, если совпадений нет).",
      ...
      arguments: [
        { name: "preservation", description: "Гарантирует, что все данные из левой (первой) таблицы попадут в отчет.", example: "FROM products p LEFT JOIN sales s ON p.id = s.p_id" },
        { name: "NULL values", description: "Если в правой таблице нет данных, столбцы заполняются NULL.", example: "WHERE o.id IS NULL -- найти пользователей без заказов" },
        { name: "OUTER", description: "Полное название - LEFT OUTER JOIN (синонимы).", example: "LEFT OUTER JOIN departments d ON ..." }
      ]
    },
    {
      id: "right_join",
      name: "RIGHT JOIN",
      category: "Операторы JOIN",
      description: "Возвращает все записи из правой таблицы и совпадающие записи из левой.",
      ...
      arguments: [
        { name: "direction", description: "Зеркальное отражение LEFT JOIN.", example: "FROM orders o RIGHT JOIN customers c ON ..." },
        { name: "usage", description: "Используется реже, так как любой RIGHT JOIN можно переписать как LEFT JOIN, поменяв таблицы местами.", example: "FROM managers m RIGHT JOIN branches b ON ..." },
        { name: "integrity", description: "Помогает найти 'осиротевшие' записи в основной справочной таблице.", example: "RIGHT JOIN roles r ON u.role_id = r.id" }
      ]
    },
    {
      id: "full_join",
      name: "FULL JOIN",
      category: "Операторы JOIN",
      description: "Возвращает все записи, если есть совпадение в одной из таблиц (комбинация LEFT и RIGHT JOIN).",
      syntax: "SELECT ... FROM t1 FULL JOIN t2 ON t1.id = t2.ref_id;",
      example: "SELECT * FROM project_a_staff FULL JOIN project_b_staff ON project_a_staff.id = project_b_staff.id;",
      arguments: [
        { name: "comprehensive", description: "Показывает полную картину из обеих таблиц, включая несовпавшие строки с обеих сторон.", example: "FULL OUTER JOIN inventory i ON ..." },
        { name: "COALESCE", description: "Часто используется вместе с COALESCE для объединения ключевых столбцов.", example: "SELECT COALESCE(a.id, b.id) ..." },
        { name: "compatibility", description: "В MySQL не поддерживается напрямую (нужно имитировать через UNION).", example: "SELECT * FROM t1 FULL JOIN t2 ON ..." }
      ]
    },
    {
      id: "cross_join",
      name: "CROSS JOIN",
      category: "Операторы JOIN",
      description: "Возвращает декартово произведение строк (каждая строка первой таблицы с каждой строкой второй).",
      syntax: "SELECT ... FROM t1 CROSS JOIN t2;",
      example: "SELECT s.size, c.color FROM sizes s CROSS JOIN colors c;",
      arguments: [
        { name: "combinations", description: "Используется для генерации всех возможных комбинаций данных.", example: "FROM months CROSS JOIN products" },
        { name: "no condition", description: "Для этого типа JOIN не указывается условие ON.", example: "SELECT * FROM t1, t2; -- старый синтаксис CROSS JOIN" },
        { name: "volume", description: "Осторожно: результат может быть очень большим (число строк t1 * число строк t2).", example: "CROSS JOIN (SELECT 1 UNION SELECT 2) as multiplier" }
      ]
    },
    {
      id: "self_join",
      ...
      example: "SELECT e.name as Emp, m.name as Manager FROM employees e JOIN employees m ON e.manager_id = m.id;",
      arguments: [
        { name: "aliases", description: "Обязательно использование разных псевдонимов для таблицы.", example: "FROM categories c1 JOIN categories c2 ON ..." },
        { name: "hierarchy", description: "Идеально подходит для иерархических структур (начальник-подчиненный, родитель-потомок).", example: "ON t1.parent_id = t2.id" },
        { name: "comparisons", description: "Сравнение строк внутри одной таблицы.", example: "WHERE t1.id <> t2.id AND t1.email = t2.email -- поиск дубликатов" }
      ]
    },
    // Битовые операторы
    {
      ...
      description: "Побитовое И (AND). Сравнивает каждый бит первого операнда с соответствующим битом второго.",
      syntax: "expression1 & expression2",
      example: "SELECT 5 & 3; -- Результат: 1 (0101 & 0011 = 0001)",
      arguments: [
        { name: "masking", description: "Используется для проверки установки определенных битов (флагов).", example: "WHERE (flags & 4) > 0" },
        { name: "integers", description: "Работает с целочисленными типами данных.", example: "SELECT user_permissions & 1" },
        { name: "permissions", description: "Часто применяется в битовых масках прав доступа.", example: "SELECT * FROM roles WHERE (mask & 2) = 2" }
      ]
    },
    {
      id: "bit_or",
      name: "|",
      category: "Битовые операторы",
      description: "Побитовое ИЛИ (OR). Результат имеет 1 в тех позициях, где хотя бы один из битов равен 1.",
      syntax: "expression1 | expression2",
      example: "SELECT 5 | 3; -- Результат: 7 (0101 | 0011 = 0111)",
      arguments: [
        { name: "combining", description: "Используется для объединения нескольких битовых флагов.", example: "UPDATE users SET flags = flags | 8" },
        { name: "options", description: "Применяется для установки дополнительных опций в поле конфигурации.", example: "SELECT status | 1 FROM devices" },
        { name: "results", description: "Результат всегда равен или больше максимального из операндов.", example: "SELECT val | 128 FROM settings" }
      ]
    },
    {
      id: "bit_xor",
      name: "^",
      category: "Битовые операторы",
      description: "Побитовое исключающее ИЛИ (XOR). Результат имеет 1 только там, где биты операндов различаются.",
      syntax: "expression1 ^ expression2",
      example: "SELECT 5 ^ 3; -- Результат: 6 (0101 ^ 0011 = 0110)",
      arguments: [
        { name: "toggling", description: "Используется для переключения (инверсии) конкретных битов.", example: "UPDATE tasks SET status = status ^ 1" },
        { name: "differences", description: "Помогает найти различающиеся биты в двух масках.", example: "SELECT mask1 ^ mask2" },
        { name: "encryption", description: "Простейшая форма симметричного шифрования (XOR-шифр).", example: "SELECT data ^ key FROM secret_table" }
      ]
    },
    {
      id: "bit_not",
      name: "~",
      category: "Битовые операторы",
      description: "Побитовое НЕ (NOT). Инвертирует все биты операнда (0 становится 1, и наоборот).",
      syntax: "~ expression",
      example: "SELECT ~5; -- Инвертирует все биты числа 5",
      arguments: [
        { name: "unary", description: "Это унарный оператор, принимающий только один операнд.", example: "SELECT ~flags" },
        { name: "complement", description: "Возвращает дополнение числа до двух.", example: "SELECT ~val + 1" },
        { name: "logic", description: "Часто используется вместе с & для очистки битов.", example: "UPDATE t SET f = f & ~4 -- сбросить 3-й бит" }
      ]
    },
    {
      id: "bit_shift_left",
      name: "<<",
      category: "Битовые операторы",
      description: "Побитовый сдвиг влево. Сдвигает биты первого операнда влево на указанное количество позиций.",
      syntax: "expression << distance",
      example: "SELECT 1 << 3; -- Результат: 8 (0001 становится 1000)",
      arguments: [
        { name: "multiplication", description: "Сдвиг влево на N позиций эквивалентен умножению на 2 в степени N.", example: "SELECT val << 1; -- Умножение на 2" },
        { name: "flags", description: "Используется для создания битовых масок по позиции.", example: "SELECT 1 << bit_pos" },
        { name: "performance", description: "Обычно выполняется быстрее, чем обычное умножение.", example: "SELECT price << 2 -- Умножение на 4" }
      ]
    },
    // Операторы присваивания
    {
      ...
      description: "Прибавляет значение к переменной и сохраняет результат (используется в процедурах и скриптах).",
      syntax: "@variable += expression",
      example: "SET @total += 100;",
      arguments: [
        { name: "shorthand", description: "Краткая запись для @v = @v + e.", example: "SET @count += 1" },
        { name: "T-SQL", description: "Активно используется в диалекте SQL Server.", example: "DECLARE @x INT = 0; SET @x += 5;" },
        { name: "accumulation", description: "Удобно для накопления суммы в циклах.", example: "SET @sum += @current_value" }
      ]
    },
    {
      id: "assign_sub",
      ...
      arguments: [
        { name: "decrement", description: "Уменьшение значения счетчика.", example: "SET @retries -= 1" },
        { name: "stock", description: "Списание остатков в процедурах.", example: "SET @stock_level -= @sold_qty" },
        { name: "shorthand", description: "Эквивалентно @v = @v - e.", example: "SET @timer -= interval" }
      ]
    },
    {
      id: "assign_mul",
      ...
      arguments: [
        { name: "scaling", description: "Изменение масштаба переменной.", example: "SET @multiplier *= 2" },
        { name: "compound", description: "Расчет сложных процентов или коэффициентов.", example: "SET @rate *= @factor" },
        { name: "shorthand", description: "Эквивалентно @v = @v * e.", example: "SET @value *= 0.5" }
      ]
    },
    {
      id: "assign_div",
      ...
      arguments: [
        { name: "reduction", description: "Последовательное деление значения.", example: "SET @x /= 2" },
        { name: "averaging", description: "Используется при расчете средних величин в скриптах.", example: "SET @val /= @divisor" },
        { name: "shorthand", description: "Эквивалентно @v = @v / e.", example: "SET @amount /= 10" }
      ]
    },
    {
      id: "assign_mod",
      ...
      example: "SET @id %= 10;",
      arguments: [
        { name: "rotation", description: "Ограничение диапазона значений переменной.", example: "SET @angle %= 360" },
        { name: "logic", description: "Используется для определения очередности или группировки в циклах.", example: "SET @group_id %= @num_groups" },
        { name: "shorthand", description: "Эквивалентно @v = @v % e.", example: "SET @counter %= 2" }
      ]
    },
    { 
      id: "avg", 
    ...
    { 
      ...
      arguments: [
        { name: "ALL | DISTINCT", description: "Определяет, учитывать ли все значения или только уникальные.", example: "SELECT CHECKSUM_AGG(DISTINCT price) FROM inventory;" },
        { name: "expression", description: "Целочисленное выражение, для которого вычисляется контрольная сумма.", example: "SELECT CHECKSUM_AGG(quantity) FROM inventory;" }
      ],
    ...
    { 
      ...
      name: "STRING_AGG()", 
      category: "Агрегатные функции", 
      description: "Объединяет строковые значения в одну строку с заданным разделителем.", 
      syntax: "STRING_AGG(expression, separator) [ <order_clause> ]",
      arguments: [
        { name: "expression", description: "Строковое выражение для объединения.", example: "SELECT STRING_AGG(name, ', ') FROM tags;" },
        { name: "separator", description: "Разделитель между значениями.", example: "SELECT STRING_AGG(name, ' | ') FROM tags;" },
        { name: "order_clause", description: "Необязательное предложение WITHIN GROUP (ORDER BY ...), определяющее порядок объединения.", example: "SELECT STRING_AGG(name, ', ' ORDER BY name) FROM tags;" }
      ],
    ...
    { 
      ...
      arguments: [
        { name: "ALL", description: "Применяет агрегатную функцию ко всем значениям. По умолчанию.", example: "SELECT COUNT(ALL role) FROM users;" },
        { name: "DISTINCT", description: "Возвращает количество уникальных ненулевых значений.", example: "SELECT COUNT(DISTINCT city) FROM users;" },
        { name: "expression", description: "Выражение для подсчета. Стандартно - имя столбца.", example: "SELECT COUNT(id) FROM products;" }
      ],
    ...
    { 
      ...
      arguments: [
        { name: "expression", description: "Числовое выражение (столбец или расчет).", example: "SELECT SUM(quantity * unit_price) FROM order_details;" }
      ],
    ...
    { 
      ...
      arguments: [
        { name: "boolean_expression", description: "Выражение, возвращающее логическое значение.", example: "SELECT EVERY(is_active) FROM users;" }
      ],
    ...
    { 
      ...
      name: "ANY() | SOME()", 
      category: "АГРЕГИРУЮЩИЕ ФУНКЦИИ", 
      description: "Возвращает true, если хотя бы одно значение в группе истинно.", 
      syntax: "ANY(boolean_expression) | SOME(boolean_expression)",
      arguments: [
        { name: "boolean_expression", description: "Выражение, возвращающее логическое значение.", example: "SELECT ANY(is_shipped) FROM orders;" }
      ],
    ...
    { 
      ...
      name: "STDDEV_POP()", 
      category: "АГРЕГИРУЮЩИЕ ФУНКЦИИ", 
      description: "Вычисляет статистическое стандартное отклонение для совокупности.", 
      syntax: "STDDEV_POP(expression)",
      arguments: [
        { name: "expression", description: "Числовое выражение.", example: "SELECT STDDEV_POP(price) FROM products;" }
      ],
    ...
    { 
      ...
      arguments: [
        { name: "expression", description: "Числовое выражение.", example: "SELECT STDDEV_SAMP(amount) FROM sales;" }
      ],
    ...
    { 
      ...
      arguments: [
        { name: "expression", description: "Числовое выражение.", example: "SELECT VAR_POP(score) FROM results;" }
      ],
    ...
    { 
      ...
      arguments: [
        { name: "expression", description: "Числовое выражение.", example: "SELECT VAR_SAMP(amount) FROM transactions;" }
      ],
    ...
    { 
      ...
      name: "GROUPING()", 
      category: "АГРЕГИРУЮЩИЕ ФУНКЦИИ", 
      description: "Указывает, является ли указанное выражение столбца в списке GROUP BY агрегированным или нет.", 
      syntax: "GROUPING(column_reference)",
      arguments: [
        { name: "column_reference", description: "Ссылка на столбец в списке GROUP BY.", example: "SELECT category, GROUPING(category) FROM products GROUP BY ROLLUP(category);" }
      ],
    ...
    { 
      ...
      arguments: [
        { name: "expression", description: "Выражение, из которого выбирается значение.", example: "SELECT category, ANY_VALUE(name) FROM products GROUP BY category;" }
      ],
      example: "SELECT category, ANY_VALUE(name) FROM products GROUP BY category;" 
    },
    { 
      id: "listagg", 
      name: "LISTAGG()", 
      ...
      syntax: "LISTAGG([ALL | DISTINCT] expression, separator [NULLS FIRST | NULLS LAST])",
      arguments: [
        { name: "expression", description: "Выражение для объединения.", example: "LISTAGG(name, ', ')" },
        { name: "separator", description: "Строка-разделитель.", example: "LISTAGG(city, '; ')" },
        { name: "NULLS FIRST | LAST", description: "Определяет порядок обработки NULL значений.", example: "LISTAGG(comment, ',' NULLS LAST)" }
      ],
      example: "SELECT category, LISTAGG(name, ', ') FROM products GROUP BY category;" 
    },
    { 
      id: "array_agg_sql", 
      name: "ARRAY_AGG()", 
      ...
      syntax: "ARRAY_AGG([DISTINCT] expression [ORDER BY sort_expression [ASC | DESC]] [NULLS FIRST | NULLS LAST])",
      arguments: [
        { name: "DISTINCT", description: "Удаляет дубликаты перед агрегацией.", example: "ARRAY_AGG(DISTINCT color)" },
        { name: "ORDER BY", description: "Сортировка элементов в массиве.", example: "ARRAY_AGG(name ORDER BY created_at DESC)" }
      ],
    ...
    { 
      ...
      syntax: "JSON_ARRAYAGG(expression [ORDER BY sort_expression [ASC | DESC]] [NULLS FIRST | NULLS LAST])",
      arguments: [
        { name: "expression", description: "Выражение для включения в массив.", example: "JSON_ARRAYAGG(price)" },
        { name: "ORDER BY", description: "Порядок элементов в массиве.", example: "JSON_ARRAYAGG(name ORDER BY name)" }
      ],
    ...
    { 
      ...
      syntax: "JSON_OBJECTAGG(key_expression : value_expression [NULL ON NULL | ABSENT ON NULL] [{WITH | WITHOUT} UNIQUE [KEYS]])",
      arguments: [
        { name: "key_expression", description: "Выражение для ключа объекта.", example: "JSON_OBJECTAGG(id : name)" },
        { name: "value_expression", description: "Выражение для значения объекта.", example: "JSON_OBJECTAGG(code : price)" },
        { name: "UNIQUE KEYS", description: "Гарантирует уникальность ключей в объекте.", example: "JSON_OBJECTAGG(k : v WITH UNIQUE KEYS)" }
      ],
    ...
    // СТРОКОВЫЕ ФУНКЦИИ
    {
      ...
      arguments: [
        { name: "character_expression", description: "Исходная строка.", example: "UPPER('sql')" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "character_expression", description: "Исходная строка.", example: "LOWER('SQL')" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "FROM start", description: "Начальная позиция (с 1).", example: "SUBSTRING('SQL' FROM 1)" },
        { name: "FOR length", description: "Количество извлекаемых символов.", example: "SUBSTRING('Database' FROM 1 FOR 4)" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "substring", description: "Искомая подстрока.", example: "POSITION('a' IN 'cat')" },
        { name: "string", description: "Строка поиска.", example: "POSITION('@' IN email)" }
      ],
    ...
    },
    {
      ...
      name: "TRIM()",
      category: "Строковые функции",
      description: "Удаляет указанные символы с начала, конца или с обеих сторон строки.",
      syntax: "TRIM([{LEADING | TRAILING | BOTH} [trim_character] FROM] character_expression)",
      arguments: [
        { name: "LEADING", description: "Удаляет символы только в начале.", example: "TRIM(LEADING '0' FROM '000123')" },
        { name: "TRAILING", description: "Удаляет символы только в конце.", example: "TRIM(TRAILING ' ' FROM 'text  ')" },
        { name: "BOTH", description: "Удаляет символы с обеих сторон.", example: "TRIM(BOTH '*' FROM '**secret**')" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "length", description: "Целевая длина строки.", example: "LPAD('5', 3, '0')" },
        { name: "pad_character", description: "Символ заполнения.", example: "LPAD('text', 10, '*')" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "decimal_places", description: "Количество знаков после запятой (по умолчанию 0).", example: "ROUND(123.456, 1) -- 123.5" }
      ],
    ...
    },
    {
      ...
      name: "WIDTH_BUCKET()",
      category: "Функции сравнения",
      description: "Распределяет значение по корзинам (buckets) в заданном диапазоне.",
    ...
    },
    {
      ...
      name: "RAND(seed)",
      category: "Прочие числовые функции",
      description: "Генерирует воспроизводимое случайное число на основе зерна (seed).",
      syntax: "RAND(seed)",
    ...
    },
    {
      ...
      name: "LOG()",
      category: "Логарифмические и экспоненциальные функции",
      description: "Возвращает логарифм числа по заданному основанию (или натуральный логарифм в некоторых диалектах).",
    ...
    // ОКОННЫЕ ФУНКЦИИ
    {
      ...
      syntax: "ROW_NUMBER() OVER ([PARTITION BY partition_expr] ORDER BY sort_expr)",
      example: "SELECT name, ROW_NUMBER() OVER(ORDER BY salary DESC) FROM employees;"
    },
    {
      id: "rank",
      name: "RANK()",
    ...
    },
    {
      ...
      syntax: "DENSE_RANK() OVER ([PARTITION BY partition_expr] ORDER BY sort_expr)",
    ...
    },
    {
      ...
      name: "EXTRACT()",
      category: "Функции даты и времени",
      description: "Извлекает определенную часть (год, месяц, день и т.д.) из значения даты или времени.",
      syntax: "EXTRACT(field FROM datetime_expression)",
      arguments: [
        { name: "field", description: "Часть даты для извлечения (YEAR, MONTH, DAY, HOUR, MINUTE, SECOND и др.).", example: "EXTRACT(YEAR FROM CURRENT_DATE)" }
      ],
    ...
    },
    {
      ...
      name: "Арифметика дат",
      category: "Функции даты и времени",
      description: "Операции сложения и вычитания для дат, меток времени и интервалов.",
      syntax: "expression { + | - } interval",
      arguments: [
        { name: "date + interval", description: "Добавление интервала к дате.", example: "CURRENT_DATE + INTERVAL '1 day'" },
        { name: "timestamp - interval", description: "Вычитание интервала из метки времени.", example: "CURRENT_TIMESTAMP - INTERVAL '2 hours'" },
        { name: "interval + interval", description: "Сложение двух интервалов.", example: "INTERVAL '1 day' + INTERVAL '2 hours'" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "expression1, expression2, ...", description: "Список выражений для проверки на NULL.", example: "COALESCE(bonus, 0)" }
      ],
      example: "SELECT name, COALESCE(phone, email, 'N/A') as contact_info FROM contacts;"
    },
    {
      id: "nullif",
      name: "NULLIF()",
      category: "Условные выражения",
      description: "Возвращает NULL, если два выражения равны, иначе первое выражение.",
      syntax: "NULLIF(expression1, expression2)",
      arguments: [
        { name: "expression1", description: "Первое выражение для сравнения.", example: "NULLIF(salary, 0)" },
        { name: "expression2", description: "Второе выражение для сравнения.", example: "NULLIF(value, -1)" }
      ],
      example: "SELECT name, salary / NULLIF(hours, 0) as hourly_rate FROM employees;"
    },
    {
      id: "ifnull_isnull",
      name: "IFNULL() | ISNULL()",
      category: "Условные выражения",
      description: "Заменяет NULL на указанное значение (зависит от диалекта: IFNULL в MySQL/SQLite, ISNULL в T-SQL).",
      syntax: "IFNULL(expression, replacement) | ISNULL(expression, replacement)",
      arguments: [
        { name: "expression", description: "Выражение, которое может содержать NULL.", example: "IFNULL(discount, 0)" },
        { name: "replacement", description: "Значение для замены, если выражение равно NULL.", example: "ISNULL(status, 'unknown')" }
      ],
      example: "SELECT product_name, IFNULL(description, 'Нет описания') FROM products;"
    },
    // ФУНКЦИИ ПРЕОБРАЗОВАНИЯ ТИПОВ
    {
      ...
      arguments: [
        { name: "expression", description: "Значение или столбец для преобразования.", example: "CAST('123' AS INTEGER)" },
        { name: "data_type", description: "Целевой тип данных (INTEGER, VARCHAR, DATE и др.).", example: "CAST(price AS VARCHAR)" },
        { name: "FORMAT", description: "Необязательная строка формата (поддерживается в некоторых диалектах).", example: "CAST(date_col AS VARCHAR FORMAT 'YYYY-MM-DD')" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "expression", description: "Числовое выражение или дата.", example: "TO_CHAR(123.45, '999.9')" },
        { name: "format_string", description: "Шаблон форматирования.", example: "TO_CHAR(CURRENT_DATE, 'DD.MM.YYYY')" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "character_expression", description: "Строка, содержащая числовое представление.", example: "TO_NUMBER('1234.56')" },
        { name: "format_string", description: "Необязательный шаблон формата.", example: "TO_NUMBER('1,234.56', '9,999.99')" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "character_expression", description: "Строка с датой.", example: "TO_DATE('2023-01-01', 'YYYY-MM-DD')" },
        { name: "format_string", description: "Шаблон формата даты.", example: "TO_DATE('01/01/23', 'DD/MM/YY')" }
      ],
    ...
    },
    {
      ...
      name: "TO_TIMESTAMP()",
      category: "Функции преобразования типов",
      description: "Преобразует строку в метку времени (timestamp) с заданным форматом.",
      syntax: "TO_TIMESTAMP(character_expression, format_string)",
      arguments: [
        { name: "character_expression", description: "Строка с датой и временем.", example: "TO_TIMESTAMP('2024-05-15 14:30:00', 'YYYY-MM-DD HH24:MI:SS')" },
        { name: "format_string", description: "Шаблон формата.", example: "TO_TIMESTAMP('01-01-2024 12:00', 'DD-MM-YYYY HH:MI')" }
      ],
      example: "SELECT TO_TIMESTAMP('2024-01-01 12:00:00', 'YYYY-MM-DD HH24:MI:SS');"
    },
    // JSON ФУНКЦИИ (SQL:2023)
    {
      ...
      syntax: "JSON_ARRAY([expression1 [, expression2, ...]] [NULL ON NULL | ABSENT ON NULL] [RETURNING data_type])",
      arguments: [
        { name: "expression", description: "Значение для включения в массив.", example: "JSON_ARRAY(1, 'two', 3.0)" },
        { name: "NULL ON NULL", description: "Сохранять NULL значения в массиве (по умолчанию).", example: "JSON_ARRAY(name NULL ON NULL)" },
        { name: "ABSENT ON NULL", description: "Игнорировать NULL значения.", example: "JSON_ARRAY(phone ABSENT ON NULL)" }
      ],
    ...
    },
    {
      ...
      category: "JSON Функции (SQL:2023)",
      description: "Создает JSON-объект из пар ключ:значение.",
      syntax: "JSON_OBJECT([key1 : value1 [, ...]] [NULL ON NULL | ABSENT ON NULL] [{WITH | WITHOUT} UNIQUE KEYS])",
      arguments: [
        { name: "key : value", description: "Пара ключ и значение для объекта.", example: "JSON_OBJECT('id' : 1, 'name' : 'Ivan')" },
        { name: "UNIQUE KEYS", description: "Гарантирует уникальность ключей в объекте.", example: "JSON_OBJECT('k' : 'v' WITH UNIQUE KEYS)" }
      ],
      example: "SELECT JSON_OBJECT('user_id' : id, 'user_name' : name) FROM employees;"
    },
    {
      id: "json_arrayagg_2023",
      name: "JSON_ARRAYAGG()",
      category: "JSON Функции (SQL:2023)",
      description: "Агрегатная функция, создающая JSON-массив из значений группы.",
      syntax: "JSON_ARRAYAGG(expression [ORDER BY sort_expression] [NULL ON NULL | ABSENT ON NULL])",
      arguments: [
        { name: "expression", description: "Выражение для агрегации.", example: "JSON_ARRAYAGG(price)" },
        { name: "ORDER BY", description: "Порядок элементов в итоговом массиве.", example: "JSON_ARRAYAGG(name ORDER BY name DESC)" }
      ],
      example: "SELECT department, JSON_ARRAYAGG(name) FROM employees GROUP BY department;"
    },
    {
      id: "json_objectagg_2023",
      name: "JSON_OBJECTAGG()",
      category: "JSON Функции (SQL:2023)",
      description: "Агрегатная функция, создающая JSON-объект из пар ключ:значение в группе.",
      syntax: "JSON_OBJECTAGG(key_expression : value_expression [NULL ON NULL | ABSENT ON NULL])",
      arguments: [
        { name: "key_expression", description: "Выражение для ключа (обычно строка).", example: "JSON_OBJECTAGG(id : name)" },
        { name: "value_expression", description: "Выражение для значения.", example: "JSON_OBJECTAGG(sku : price)" }
      ],
      example: "SELECT category, JSON_OBJECTAGG(product_id : product_name) FROM products GROUP BY category;"
    },
    {
      id: "json_query",
      name: "JSON_QUERY()",
      category: "JSON Функции (SQL:2023)",
      description: "Извлекает JSON-объект или массив из JSON-строки по заданному пути.",
      syntax: "JSON_QUERY(json_expr, path_expr [RETURNING data_type])",
      arguments: [
        { name: "json_expr", description: "Исходная JSON-строка или столбец.", example: "JSON_QUERY(data, '$.address')" },
        { name: "path_expr", description: "SQL/JSON путь (JSONPath).", example: "JSON_QUERY(info, '$.tags')" }
      ],
    ...
    },
    {
      ...
      name: "JSON_VALUE()",
      category: "JSON Функции (SQL:2023)",
      description: "Извлекает скалярное (одиночное) значение из JSON-строки по заданному пути.",
      syntax: "JSON_VALUE(json_expr, path_expr [RETURNING data_type])",
      arguments: [
        { name: "json_expr", description: "Исходная JSON-строка.", example: "JSON_VALUE(data, '$.name')" },
        { name: "path_expr", description: "Путь к конкретному значению.", example: "JSON_VALUE(config, '$.timeout')" }
      ],
    ...
    },
    {
      ...
      name: "JSON_EXTRACT()",
      category: "JSON Функции (SQL:2023)",
      description: "Извлекает данные из JSON (широко используется в MySQL/SQLite).",
      syntax: "JSON_EXTRACT(json_expression, path_expression)",
      arguments: [
        { name: "path_expression", description: "Путь в формате '$.key'.", example: "JSON_EXTRACT(info, '$.items[0]')" }
      ],
    ...
    // ARRAY ФУНКЦИИ (SQL:2023)
    {
      ...
      arguments: [
        { name: "array_expression", description: "Массив, размер которого нужно вычислить.", example: "CARDINALITY(ARRAY[1, 2, 3])" }
      ],
    ...
    },
    {
      ...
      category: "Array Функции (SQL:2023)",
      description: "Собирает значения в массив.",
      syntax: "ARRAY_AGG([DISTINCT] expression [ORDER BY sort_expr] [NULLS FIRST | NULLS LAST])",
      arguments: [
        { name: "expression", description: "Выражение для включения в массив.", example: "ARRAY_AGG(name)" },
        { name: "ORDER BY", description: "Сортировка элементов внутри массива.", example: "ARRAY_AGG(score ORDER BY score DESC)" }
      ],
      example: "SELECT department, ARRAY_AGG(name ORDER BY hire_date) FROM employees GROUP BY department;"
    },
    {
      id: "array_append",
      name: "ARRAY_APPEND()",
      ...
      arguments: [
        { name: "array_expression", description: "Исходный массив.", example: "ARRAY_APPEND(tags, 'new-tag')" },
        { name: "element_expression", description: "Элемент для добавления.", example: "ARRAY_APPEND(items, 100)" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "element_expression", description: "Элемент для добавления.", example: "ARRAY_PREPEND('first', list)" },
        { name: "array_expression", description: "Целевой массив.", example: "ARRAY_PREPEND(1, ARRAY[2, 3])" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "array_expr1", description: "Первый массив.", example: "ARRAY_CAT(ARRAY[1,2], ARRAY[3,4])" },
        { name: "array_expr2", description: "Второй массив.", example: "ARRAY_CAT(tags1, tags2)" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "WITH ORDINALITY", description: "Добавляет столбец с порядковым номером элемента.", example: "UNNEST(tags) WITH ORDINALITY" }
      ],
    ...
    },
    {
      ...
      arguments: [
        { name: "num_value", description: "Количество удаляемых элементов.", example: "TRIM_ARRAY(ARRAY[1, 2, 3, 4], 2) -- [1, 2]" }
      ],
    ...
    },
    {
      ...
      name: "CURRENT_PATH",
      category: "Функции сессии/контекста",
      description: "Возвращает список схем, используемых для поиска функций и операторов.",
      syntax: "CURRENT_PATH",
    ...
    },
    {
      ...
      name: "VALUE",
      category: "Функции сессии/контекста",
      description: "Используется в контексте проверок (CHECK) или доменов для ссылки на текущее значение.",
      syntax: "VALUE",
    ...
    // ФУНКЦИИ ГРУППИРОВКИ
    {
      ...
      description: "Указывает, является ли указанный столбец в списке GROUP BY агрегированным (часть ROLLUP/CUBE) или нет.",
      syntax: "GROUPING(column_reference)",
      arguments: [
        { name: "column_reference", description: "Ссылка на столбец из списка GROUP BY.", example: "GROUPING(category)" }
      ],
      example: "SELECT category, SUM(price), GROUPING(category) FROM products GROUP BY ROLLUP(category);"
    },
    {
      id: "group_concat",
      name: "GROUP_CONCAT()",
      category: "Функции группировки",
      description: "Объединяет значения из группы в одну строку с разделителем (MySQL/SQLite).",
      syntax: "GROUP_CONCAT([DISTINCT] expression [ORDER BY sort_expr] [SEPARATOR separator_string])",
      arguments: [
        { name: "expression", description: "Выражение для объединения.", example: "GROUP_CONCAT(name)" },
        { name: "SEPARATOR", description: "Символ-разделитель (по умолчанию запятая).", example: "GROUP_CONCAT(city SEPARATOR '; ')" }
      ],
      example: "SELECT department_id, GROUP_CONCAT(name) FROM employees GROUP BY department_id;"
    },
    {
      id: "listagg_group",
      name: "LISTAGG()",
      category: "Функции группировки",
      description: "Агрегатная функция для объединения строк в одну с заданным порядком (Oracle/SQL Standard).",
      syntax: "LISTAGG([DISTINCT] expression, separator) WITHIN GROUP (ORDER BY sort_expr)",
      arguments: [
        { name: "expression", description: "Строковое выражение для объединения.", example: "LISTAGG(name, ', ')" },
        { name: "WITHIN GROUP", description: "Определяет порядок элементов в итоговой строке.", example: "WITHIN GROUP (ORDER BY hire_date)" }
      ],
      example: "SELECT job_id, LISTAGG(last_name, '; ') WITHIN GROUP (ORDER BY last_name) FROM employees GROUP BY job_id;"
    },
